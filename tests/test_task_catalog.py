...
def test_task_catalog_classifies_core_legacy_and_infra_tasks() -> None:
    assert task_spec("collect.prices").lineage == "infrastructure_only"
    assert task_spec("collect.prices").production_allowed is True
...
def test_task_catalog_payload_is_dashboard_safe() -> None:
    ...
    assert any(item["canonical_name"] == "collect.prices" for item in payload)

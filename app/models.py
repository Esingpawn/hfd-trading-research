...
class TradeCandidate(Base):
    ...
    __table_args__ = (
        UniqueConstraint("candidate_key", name="uq_trade_candidates_candidate_key"),
        Index("ix_trade_candidates_lineage_status", "lineage", "status"),
        Index("ix_trade_candidates_symbol_status", "symbol", "status"),
        Index("ix_trade_candidates_strategy_setup", "strategy_id", "setup_time"),
    )

    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    candidate_key: Mapped[str] = mapped_column(String(200), unique=True, index=True)
    source_type: Mapped[str] = mapped_column(String(64), index=True, default="darkflow_interaction")
    source_interaction_id: Mapped[str | None] = mapped_column(String(36), index=True)
    lineage: Mapped[str] = mapped_column(String(64), index=True, default="core_darkflow_v2")
    strategy_family: Mapped[str] = mapped_column(String(80), index=True)
    ...
    market_state: Mapped[str] = mapped_column(String(80), index=True)
    setup_time: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), index=True)
    entry_price: Mapped[float] = mapped_column(Float)
    ...
    status: Mapped[str] = mapped_column(String(32), index=True, default="research_blocked")
    promotion_status: Mapped[str] = mapped_column(String(32), index=True, default="blocked")
    anti_repaint_status: Mapped[str] = mapped_column(String(32), index=True, default="missing")
    shadow_status: Mapped[str] = mapped_column(String(32), index=True, default="not_started")
    paper_eligible: Mapped[bool] = mapped_column(default=False)
    live_eligible: Mapped[bool] = mapped_column(default=False)
    blockers: Mapped[list[str]] = mapped_column(JSON, default=list)
    promotion_blockers: Mapped[list[str]] = mapped_column(JSON, default=list)
    ...
    materialized_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, index=True)
